...
class PersistentQSQLite:
    ...
    def mark_failed(self, job_id: str) -> None:
        """Mark job as FAILED or UNPROCESSABLE based on retries."""
        now = datetime.now().isoformat()
        with self._lock:
            with self._get_conn() as conn:
                cursor = conn.cursor()
                cursor.execute("""
                    ...
                    WHERE id = ? AND consumer_id = ? AND retries < 3
                """, (now, job_id, self.consumer_id))
                cursor.execute("""
                    UPDATE jobs 
        ...
        logger.warning(f"Marked job_id failed: {job_id}")
    ...
    def check_stalled_jobs(self, cursor: Optional[sqlite3.Cursor] = None) -> None:
        ...
        """, (now, (datetime.now() - timedelta(minutes=5)).isoformat()))
